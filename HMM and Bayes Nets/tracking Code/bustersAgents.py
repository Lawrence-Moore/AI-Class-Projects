...
class GreedyBustersAgent(BustersAgent):
	...
	def chooseAction(self, gameState):
		...
		#Go through each ghost's probablity distribution
		#store the most likely position of each ghost in a list
		mostLikelyPos = []
		for ghostInfo in livingGhostPositionDistributions:
			highestProb = 0
			#go through each legal position, find the most liekyl
			for pos in ghostInfo.sortedKeys():
				prob = ghostInfo[pos]
				#updates the information accordingly
				if prob > highestProb:
					highestPos = pos
					highestProb = prob
			#store the most likely position for the ghost in the list
			mostLikelyPos.append(highestPos)

		#find which action brings us the closest to a ghost
		minDist = sys.maxint
		minAction = 'Stop'
		for action in legal:
			succPos = Actions.getSuccessor(pacmanPosition, action)
			#find the cloest ghost based on this move
			for ghost in mostLikelyPos:
				distance = self.distancer.getDistance(succPos, ghost)
				#only update if this distance is the overall closest to any ghost
				if distance < minDist:
					minDist = distance
					minAction = action
		#return the minimum action
		return minAction